...
BEGIN
    IF :NEW.budget <= 0 THEN
        RAISE_APPLICATION_ERROR(-20010, 'Budget invalide: le budget ne peut pas �tre inf�rieur � 0');
    END IF;

    IF :NEW.date_fin < :NEW.date_debut THEN
        RAISE_APPLICATION_ERROR(-20020, 'Dates invalides: La date de fin ne peut pas �tre plus petite que celle de d�but');
    END IF;
END;
/
